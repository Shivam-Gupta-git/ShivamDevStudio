...
const INITIAL_PROBLEMS = JSON.parse(JSON.stringify(problems))

export function getProblemById(id) {
  return problems.find((p) => p.id === id) ?? null
}

export function addOrUpdateProblem(problemData) {
  const index = problems.findIndex((p) => p.id === problemData.id)
  if (index >= 0) {
    ...
    const newProblem = {
      ...problemData,
      id: problemData.id || `custom-${Date.now()}`,
    }
...
export function resetProblems() {
  problems.length = 0
  problems.push(...JSON.parse(JSON.stringify(INITIAL_PROBLEMS)))
  return problems
}
